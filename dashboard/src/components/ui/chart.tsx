...
function ChartTooltipContent({
  ...
  formatter?: (value: NonNullable<ChartPayloadItem["value"]>, name: NonNullable<ChartPayloadItem["name"]>, item: ChartPayloadItem, index: number, payload?: Record<string, unknown>) => React.ReactNode;
    ...
    <div className={cn("grid min-w-32 items-start gap-1.5 rounded-lg border bg-background px-2.5 py-1.5 text-xs shadow-xl", className)}>
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          ...
          return (
            <div key={item.dataKey ?? index} className={cn("flex w-full flex-wrap items-stretch gap-2 [&>svg]:size-2.5 [&>svg]:text-muted-foreground", indicator === "dot" && "items-center")}>
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  ...
                  <div className={cn("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center")}>
                    ...
                    {item.value !== undefined && <span className="font-mono font-medium tabular-nums text-foreground">{typeof item.value === "number" ? item.value.toLocaleString() : item.value}</span>}
                  </div>
                </>
              )}
            </div>
