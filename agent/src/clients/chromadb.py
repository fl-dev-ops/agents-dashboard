...
DEFAULT_CHROMA_RETRIEVAL_CONCURRENCY = 4


def _build_retrieval_semaphore() -> asyncio.Semaphore:
    raw_limit = os.getenv(
        "CHROMA_RETRIEVAL_CONCURRENCY",
        str(DEFAULT_CHROMA_RETRIEVAL_CONCURRENCY),
    )
    try:
        limit = int(raw_limit)
    except ValueError:
        limit = DEFAULT_CHROMA_RETRIEVAL_CONCURRENCY
    return asyncio.Semaphore(max(limit, 1))


_retrieval_semaphore = _build_retrieval_semaphore()
